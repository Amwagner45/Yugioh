        ...
        with get_db_connection() as conn:
            if self.id and self.id > 0:
                ...
                """, (self.username, self.email, self.display_name, 
                     json.dumps(self.preferences), self.id))
            else:
                # Insert new user
                if self.id == 1:
                    ...
                    conn.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?)
                    """, (1, self.username, self.email, self.display_name, 
...
class Binder:
    ...
    def get_by_uuid(cls, binder_uuid: str) -> Optional['Binder']:
        """Get binder by UUID"""
        with get_db_connection() as conn:
            cursor = conn.execute("SELECT * FROM binders WHERE uuid = ?", (binder_uuid,))
            row = cursor.fetchone()
        ...
        with get_db_connection() as conn:
            if self.id:
                ...
                conn.execute("""
                    ...
                    WHERE id = ?
                """, (self.name, self.description, json.dumps(self.tags), 
                     self.is_default, self.id))
            else:
                # Insert new binder
                cursor = conn.execute("""
                    INSERT INTO binders (uuid, user_id, name, description, tags, is_default)
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (self.uuid, self.user_id, self.name, self.description, 
                     json.dumps(self.tags), self.is_default))
                self.id = cursor.lastrowid
            conn.commit()
        return self
        ...
        with get_db_connection() as conn:
            if self.id:
                ...
                conn.execute("""
                    ...
                    WHERE id = ?
                """, (self.quantity, self.set_code, self.rarity, 
                     self.condition, self.notes, self.id))
            else:
                # Insert new entry
                cursor = conn.execute("""
                    INSERT INTO binder_cards (binder_id, card_id, quantity, set_code, rarity, condition, notes)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                """, (self.binder_id, self.card_id, self.quantity, self.set_code, 
                     self.rarity, self.condition, self.notes))
                self.id = cursor.lastrowid
            conn.commit()
        return self
        ...
        with get_db_connection() as conn:
            if self.id:
                ...
                conn.execute("""
                    ...
                    WHERE id = ?
                """, (self.name, self.description, self.format, json.dumps(self.tags),
                     self.notes, self.is_valid, json.dumps(self.validation_errors),
                     self.binder_id, self.id))
            else:
                # Insert new deck
                cursor = conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (self.uuid, self.user_id, self.binder_id, self.name, self.description,
                     self.format, json.dumps(self.tags), self.notes, self.is_valid,
        ...
        with get_db_connection() as conn:
            if self.id:
                ...
                cursor = conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?)
                """, (self.deck_id, self.card_id, self.section, self.quantity, self.order_index))
                self.id = cursor.lastrowid
            conn.commit()
        return self
