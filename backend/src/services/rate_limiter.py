...
class RateLimiter:
    ...
    async def check_rate_limit(
        self, client_ip: str, endpoint: str, is_external_api: bool = True
        ...
        if client_key in self.backoff_until:
            if current_time < self.backoff_until[client_key]:
                retry_after = (
                    self.backoff_until[client_key] - current_time
                ).total_seconds()
                return False, retry_after
            else:
                # Backoff period expired, reset
    ...
    async def record_failure(
        ...
        backoff_seconds = min(self.backoff_factor**failure_count, self.max_backoff)

        # Set backoff period
        self.backoff_until[client_key] = datetime.now() + timedelta(
        ...
        if is_external_api and self.global_failure_count >= 3:
            global_backoff_seconds = min(
                self.backoff_factor**self.global_failure_count, self.max_backoff
            )
            self.global_backoff_until = datetime.now() + timedelta(
                seconds=global_backoff_seconds
            )
    ...
    def get_stats(self) -> Dict:
        ...
        active_backoffs = sum(
            1
            for backoff_time in self.backoff_until.values()
        ...
        return {
            ...
            "global_requests_last_minute": sum(
                1 for req_time in self.global_request_times if req_time > cutoff
            ),
            ...
            "global_backoff_until": (
                self.global_backoff_until.isoformat()
                if self.global_backoff_until
                else None
            ),
        }
...
async def record_request_success(
    client_ip: str, endpoint: str, is_external_api: bool = True
):
    """Record a successful request"""
