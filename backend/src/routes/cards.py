    ...
    try:
        ...
        if atk_min is not None or atk_max is not None:
            filtered_data = [
                ...
                if card.get("atk") is not None and (
                    (atk_min is None or card["atk"] >= atk_min) and
                    (atk_max is None or card["atk"] <= atk_max)
                )
            ]
        ...
        if def_min is not None or def_max is not None:
            filtered_data = [
                card for card in filtered_data
                if card.get("def") is not None and (
                    (def_min is None or card["def"] >= def_min) and
                    (def_max is None or card["def"] <= def_max)
                )
            ]
        ...
        # Cache the results
        try:
            await cache_service.cache_card_search_results(cache_key_params, filtered_data)
        except Exception as e:
        ...
        try:
            ...
            if name:
                fallback_data = [
                    card for card in fallback_data 
    ...
    try:
        ...
        for card in card_data:
            ...
            if "race" in search_fields and query_lower in card.get("race", "").lower():
                match_found = True
            if "attribute" in search_fields and query_lower in card.get("attribute", "").lower():
                match_found = True
        ...
        return CardSearchResponse(
            data=[],
            count=0,
            error=error_handler.create_standardized_error(e, "advanced_search")["message"],
            cached=False
        )
...
async def get_available_filters(request: Request):
    ...
    return {
        "types": [
            ...
            "Spell Card", "Trap Card", "Pendulum Effect Monster"
        ],
        "races": [
            "Warrior", "Spellcaster", "Dragon", "Beast", "Fiend",
            "Machine", "Aqua", "Pyro", "Rock", "Winged Beast",
            "Plant", "Insect", "Thunder", "Dinosaur", "Beast-Warrior",
            "Zombie", "Reptile", "Psychic", "Divine-Beast", "Creator God",
            "Wyrm", "Cyberse"
        ],
        "attributes": [
        ...
        "sort_options": [
            "name", "atk", "def", "level", "id"
        ],
        "search_fields": [
            "name", "desc", "type", "race", "attribute"
        ]
    }
