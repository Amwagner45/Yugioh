...
def show_database_info():
    ...
    try:
        with get_db_connection() as conn:
            ...
            # Show schema version
            cursor = conn.execute("SELECT version, description, applied_at FROM schema_version ORDER BY version DESC LIMIT 1")
            version_info = cursor.fetchone()
            if version_info:
                print(f"Schema Version: {version_info[0]} - {version_info[1]} (Applied: {version_info[2]})")
